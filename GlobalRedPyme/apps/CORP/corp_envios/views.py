...
from .serializers import EnviosSerializer
...
def envios_list(request):
    ...
    if request.method == 'POST':
        try:
            ...
            query = Envios.objects.filter(**filters).order_by('-created_at')
            serializer = EnviosSerializer(query[offset:limit], many=True)

...
from ...GDE.gde_gestionEntrega.models import Oferta as GDE_Oferta
from ...GDE.gde_gestionEntrega.serializers import GestionOfertaSerializer
from .serializers import OfertasSerializer, OfertasListarSerializer, GestionOfertaCreateSerializer, \
...
import openpyxl
# logs
from ...CENTRAL.central_logs.methods import createLog, datosTipoLog, datosGestionOfertaGDO
...
def generarOferta_list(request):
    """
    Este metodo sirve para listar las ofertas de la tabla de ofertas de la base de datos gdo
    @type request: El campo request recibe los campos negocio, cliente, page, page_size
    @rtype: Devuelve una lista de ofertas, caso contrario devuelve el error generado
    """
...
def generarOferta_findOne(request, pk):
    """
    Este metodo sirve para obetner una oferta de la tabla oferta de la base datos gdo
    @type pk: El campo pk recibe el id de la oferta
    @type request: El campo request no recibe nada
    @rtype: DEvuelve el registro obtenido, caso ontrario devuelve el error generado
    """
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
...
def generarOferta_create(request):
    """
    Est metodo sirve para crear una oferta en la tabla oferta de la base datos gdo
    @type request: El campo request recibe los campos de la tabla oferta
    @rtype: DEvuelve el registro de la oferta generada, caso contrario devuelve el error generado
    """
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
...
def generarOferta_update(request, pk):
    """
    Este metodo sirve para actualizar la oferta generada de la tabla oferta de la base datos gdo
    @type pk: El campo pk recibe el id de la oferta generada
    @type request: El campo request recibe los campos de la tabla oferta y de detalles
...
def generarOferta_delete(request, pk):
    """
    Este metodo sirve para eliminar la oferta generada de la tabla oferta generada de la base datos gdo
    @type pk: El campo pk recibe el id de la oferta generada
    @type request: El campo request no recibe nada
    @rtype: Devuevl la oferta generada eliminada
    """
    ...
    try:
        ...
        if request.method == 'DELETE':
            serializer = GestionOfertaCreateSerializer(query, data={'state': '0', 'updated_at': str(nowDate)},
                                                       partial=True)
            if serializer.is_valid():
                serializer.save()
                createLog(logModel, serializer.data, logTransaccion)
...
def detalles_list(request, pk):
    """
    Este metodo sirve para listar los detalles de las ofertas de la tabla detalles de la base datos gdo
    @type pk: El campo pk recibe el id de la tabla oferta
    @type request: El campo request no recibe
    @rtype: DEvuelve una lista de detalles de las ofertas, caso contario devuelve el error generado
    """
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
